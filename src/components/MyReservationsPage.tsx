...
const MyReservationsPage: React.FC<MyReservationsPageProps> = ({ onClose, loggedInUserInfo, onLogout }) => {
  ...
  const handleWithdraw = async () => {
    ...
    if (!window.confirm('정말로 탈퇴하시겠습니까? 모든 예약 정보와 계정이 삭제됩니다.')) return;
    ...
    try {
      // 2) 데이터 삭제 (예약 -> 유저 정보)
      // 예약 삭제
      await supabase.from('reservations').delete().eq('userId', loggedInUserInfo!.uid);
      // 유저 테이블 삭제
      await supabase.from('users').delete().eq('id', loggedInUserInfo!.uid);

      // 3) 로그아웃 (실제 Auth 계정 삭제는 클라이언트 SDK로 어려우므로, 데이터 삭제 후 로그아웃 처리하여 접근 차단)
      // *참고: Auth 계정 자체 삭제는 보통 서버 사이드(Admin SDK)가 필요합니다. 
      // 여기서는 '탈퇴'의 효과(데이터 삭제 및 로그아웃)를 냅니다.
